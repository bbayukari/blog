...
def linear_loss_no_intercept(para, data):
    return statistic_model_pybind.linear_loss_no_intercept(para, np.zeros(0), data)
    #return np.sum(np.square(data.y - data.x @ para))

def linear_grad_no_intercept(para, data, compute_para_index):
    return statistic_model_pybind.linear_gradient_no_intercept(para, np.zeros(0), data, compute_para_index)
